...
const CustomLink = ({ href, title, className = "" }) => {
    ...
    return (
        <Link href={href} className={`${className} relative group`}>    {/*group is used to make hover active or inactive in navbar by following span group */}
            {title}

            <span className={`h-[1px] inline-block absolute
...
const CustomMobileLink = ({ href, title, className = "",toggle }) => {
    ...
    return (
        ...
        <button href={href} className={`${className} relative group text-light dark:text-dark my-2`} onClick={HandleClick}>    {/*group is used to make hover active or inactive in navbar by following span group */}
            {title}
        ...
        <header className='relative z-10 flex items-center justify-between w-full px-32 font-medium shadow py-7 lg:px-16 md:px-12 sm:px-8'>
            ...
            <div className='flex items-center justify-between w-full lg:hidden'>
                ...
                <nav className='flex flex-wrap items-center justify-center '>
                    <motion.a href="https://www.linkedin.com/in/prashantkumar89" target='_blank'
                        whileHover={{ y: -2 }}
                        whileTap={{ scale: 0.9 }}
                        className='w-6 mx-3'
                    ><LinkedInIcon /></motion.a>
                    <motion.a href="https://github.com/Prashantkry" target='_blank'
                        whileHover={{ y: -2 }}
            ...
            <button className='flex-col items-center justify-center hidden lg:flex' onClick={handleClick}>
                <span className={`bg-dark transition-all duration-300 ease-out dark:bg-light block h-0.5 w-6 rounded-sm ${isOpen ? 'rotate-45 translate-y-1' : '-translate-y-0.5'}`}></span>
